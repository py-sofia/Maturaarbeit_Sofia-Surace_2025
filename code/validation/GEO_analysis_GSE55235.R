...
gset <- getGEO("GSE55235", GSEMatrix =TRUE, AnnotGPL=TRUE)
...
gsms <- "000000000022222222221111111111"
...
gs <- factor(sml)
groups <- make.names(c("healthy","RA","osteo"))
levels(gs) <- groups
gset$group <- gs
...
par(mar=c(7,4,2,1))
title <- paste ("GSE55235", "/", annotation(gset), sep ="")
...
par(mar=c(4,4,2,1))
title <- paste ("GSE55235", "/", annotation(gset), " value distribution", sep ="")
...
plotSA(fit2, main="Mean variance trend, GSE55235")
